...
pub struct ColStats {
    ...
    nmax: f64,
    sum: f64,
    // Welford running mean/variance over the numeric values (n == count while
    // the column is still numeric).
    mean: f64,
    m2: f64,
...
impl ColStats {
    ...
    pub fn to_row(&self, field_name: &str) -> Vec<Field<'static>> {
        ...
        if self.numeric && self.count > 0 {
            ...
            row.push(if self.count >= 2 {
                Field::Num((self.m2 / (self.count - 1) as f64).sqrt())
            } else {
                Field::Str("")
            });
        } else {
