...
fn statements_after_a_sort_see_the_full_number() {
    ...
    // with its full value, not the 15-digit output text: the output
    // rounds, the pipeline does not.
    let input = "v\n1.0000000000000007\n1.0000000000000004\n";
    assert_eq!(
        run_checked(
            "add v = num(v) | sort v | select v > 1.0000000000000005",
            input
        ),
        "v\n1\n"
    );
    assert_eq!(
        run_checked(
            "add v = num(v) | sort v | add d = round((v - 1) * 10000000000000000) | cols d",
            input
        ),
        "d\n4\n7\n"
    );
    ...
    assert_eq!(
        run_checked(
            "add v = num(v) | sort v=r | select v > 1.0000000000000005 | head 1",
            input
        ),
        "v\n1\n"
    );
